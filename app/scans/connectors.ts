import type { Company, Job, SourceProvider } from "./types";
...
export type NormalizedConnectorJob = Pick<
  Job,
  | "companyId"
  | "externalJobId"
  | "sourceProvider"
...
export type ConnectorPreviewDiff = {
...
export type ConnectorFetchPreview = {
  plan: ConnectorPlan;
  fetchedAt: string;
  totalFetched: number;
  totalRelevant: number;
  filteredOut: number;
  newJobs: ConnectorPreviewDiff[];
  existingJobs: ConnectorPreviewDiff[];
  missingExistingJobs: Array<Pick<Job, "id" | "externalJobId" | "title" | "status">>;
};
...
function inferRemoteType(content: string): Job["remoteType"] {
...
function inferRemoteTypeFromFields(title: string, location: string, description: string): Job["remoteType"] {
...
function inferEmploymentType(content: string): Job["employmentType"] {
  const lower = content.toLowerCase();
...
function buildWorkdayEndpoint(company: Company) {
  try {
...
function workdaySitePath(company: Company) {
  try {
    const url = new URL(company.careersUrl);
    const pathParts = url.pathname.split("/").filter(Boolean);
...
export function parseHtmlJobs(html: string, company?: Company): unknown[] {
...
export function parseProviderHtmlJobs(html: string, company: Company): unknown[] {
...
export function buildConnectorPlan(company: Company): ConnectorPlan {
...
export function normalizeGreenhouseJob(rawJob: unknown, company: Company): NormalizedConnectorJob {
...
export function normalizeLeverJob(rawJob: unknown, company: Company): NormalizedConnectorJob {
  const raw = asRecord(rawJob);
...
export function normalizeAshbyJob(rawJob: unknown, company: Company): NormalizedConnectorJob {
  const raw = asRecord(rawJob);
  const title = asString(raw.title);
...
export function normalizeWorkdayJob(rawJob: unknown, company: Company): NormalizedConnectorJob {
  const raw = asRecord(rawJob);
  const title = asString(raw.title);
...
export function normalizeHtmlJob(rawJob: unknown, company: Company): NormalizedConnectorJob {
  const raw = asRecord(rawJob);
...
export function normalizeConnectorJob(rawJob: unknown, company: Company): NormalizedConnectorJob {
...
export function normalizeAdzunaPayload(payload: unknown, company: Company): NormalizedConnectorJob[] {
...
export function normalizeWorkablePayload(payload: unknown, company: Company): NormalizedConnectorJob[] {
...
export function normalizeWorkableAggregatePayload(payload: unknown, company: Company): NormalizedConnectorJob[] {
  const jobs = asRecord(payload).jobs;
  if (!Array.isArray(jobs)) return [];
...
export function normalizeConnectorPayload(payload: unknown, company: Company): NormalizedConnectorJob[] {
...
export function buildConnectorFetchPreview(
  plan: ConnectorPlan,
  ...
  const existingIds = new Set(
    existingJobs
      .filter((job) => job.companyId === plan.companyId && job.sourceProvider === plan.provider)
      .map((job) => job.externalJobId)
  );
  const fetchedIds = new Set(normalizedJobs.map((job) => job.externalJobId));
  const diffs = normalizedJobs.map((job): ConnectorPreviewDiff => ({
    ...
    status: existingIds.has(job.externalJobId) ? "existing" : "new",
  }));
  ...
  return {
    ...
    fetchedAt,
    totalFetched,
    totalRelevant: normalizedJobs.length,
    filteredOut,
    newJobs: diffs.filter((job) => job.status === "new"),
    existingJobs: diffs.filter((job) => job.status === "existing"),
    missingExistingJobs: existingJobs
      .filter((job) => (
        job.companyId === plan.companyId &&
        job.sourceProvider === plan.provider &&
        (job.status === "new" || job.status === "saved" || job.status === "reviewed") &&
