...
export type OccupationClassifierInput = Pick<
  NormalizedConnectorJob,
  "title" | "department" | "descriptionText" | "companyName" | "location" | "remoteType" | "salaryText"
> & {
  sourceKind?: string;
  sourceName?: string;
};
...
const laneRules: LaneRule[] = [
  ...
  },
  {
    ...
    title: [/\b(broadcast producer|content producer|video producer|live streaming producer|podcast producer)\b/],
    task: [/\b(video|broadcast|content production|post production|shoot|edit|script|storyboard|production schedule)\b/],
    adjacent: ["digital-production", "social-creative"],
  },
  {
    lane: "digital-production",
    title: [/\b(digital producer|integrated producer|interactive producer|web producer|campaign producer|brand producer)\b/],
  ...
  },
  {
    ...
    title: [/\b(software engineer|frontend engineer|backend engineer|full stack|developer|architect|machine learning engineer|data scientist)\b/],
  ...
  },
  {
    ...
    title: [/\b(data operations|it operations|systems analyst|data analyst|infrastructure|compute|cloud inference|network|security operations|data acquisition)\b/],
  ...
  },
  {
    ...
    title: [/\b(finance|fp&a|fp a|business operations|business strategy|strategy manager|chief of staff|sourcing|procurement|business transformation)\b/],
  ...
  },
  {
    ...
    title: [/\b(administrative|admin assistant|file clerk|office manager|coordinator)\b/],
...
const industrySignals = [
  /\b(ai|artificial intelligence|machine learning|llm|generative ai)\b/,
  /\b(app|mobile|saas|software|platform|product)\b/,
  /\b(blockchain|crypto|web3|fintech)\b/,
  /\b(advertising|agency|brand|campaign|media|content|creative)\b/,
];

export function occupationLanePolarity(lane: OccupationLane) {
  if ([
    "creative-writing",
    "art-direction",
    "visual-design",
...
export function isOccupationSafetyBlocked(classification: OccupationClassification) {
  return occupationLanePolarity(classification.lane) === "wrong_lane" && classification.confidence !== "low";
}
...
export function classifyOccupation(input: OccupationClassifierInput): OccupationClassification {
  ...
  const combined = normalize(`${input.title} ${input.department} ${input.descriptionText} ${input.companyName} ${input.sourceName ?? ""}`);
  ...
  const taskSignals = laneRules.flatMap((rule) => matchesAny(description, rule.task).map(() => rule.lane));
  const industryEvidence = industrySignals.filter((pattern) => pattern.test(combined)).map((pattern) => `industry/domain signal: ${pattern.source}`);
  ...
  if (!winner || winner.score <= 0) {
    return {
      ...
      confidence: "low",
      source: "unknown",
      evidence: industryEvidence.slice(0, 3),
      disqualifiers: [],
      taskSignals: Array.from(new Set(taskSignals)).slice(0, 6),
      adjacentLanes: [],
    };
  }
