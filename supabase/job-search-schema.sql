...
create table if not exists public.job_search_settings (
  ...
  digest_cadence text not null default 'weekdays' check (digest_cadence in ('manual', 'daily', 'weekdays', 'weekly')),
...
create table if not exists public.job_search_jobs (
  ...
  source_provider text not null check (source_provider in ('greenhouse', 'lever', 'ashby', 'icims', 'workday', 'magnit', 'html')),
...
insert into public.job_search_companies (
  ...
  industry_bucket, remote_likelihood, notes, status, last_successful_scan, last_error
) values
  ('company-ibc', 'default', 'IBC', 'https://example.com', 'https://example.com/careers', 'greenhouse', 'ibc', 'creative agency', 84, 'Producer role reads like post-production operations ownership, not commodity video production.', 'active', '2026-06-04T08:20:00-06:00', null),
  ('company-nova', 'default', 'Nova Systems Studio', 'https://example.com', 'https://example.com/jobs', 'lever', 'nova', 'internal creative studio', 72, 'Strong brand and launch work. Watch for program roles that avoid engineering management.', 'active', '2026-06-04T08:22:00-06:00', null),
  ('company-left-field', 'default', 'Left Field Labs', 'https://example.com', 'https://example.com/careers', 'ashby', 'leftfield', 'design agency', 55, 'Do-not-apply until manually reactivated.', 'do_not_apply', '2026-06-04T08:24:00-06:00', null),
  ('company-chain', 'default', 'Chainlight', 'https://example.com', 'https://example.com/open-roles', 'html', 'chainlight-careers', 'web3', 88, 'Crypto/web3 roles can fit when production authority is explicit.', 'active', '2026-06-04T08:27:00-06:00', 'Generic HTML fallback missed salary block on latest scan.')
on conflict (id) do nothing;
...
insert into public.job_search_jobs (
  ...
  ('job-ibc-producer', 'default', 'company-ibc', 'gh-74219', 'greenhouse', 'https://example.com/source/ibc-producer', 'https://example.com/apply/ibc-producer', 'Remote Producer', 'IBC', 'Remote, US', 'remote', 'full-time', 'Content Studio', 155000, 175000, '$155k-$175k base', 'Own post production flow, creative feedback, version control, campaign delivery, and cross-functional studio operations across distributed teams.', '2026-06-04T07:42:00-06:00', '2026-06-04T08:20:00-06:00', 'archived', 92, 'A', 'Strong match: senior production ownership, remote, post flow chaos, and compensation signal all line up.', array['Confirm authority level', 'Find actual production lead'], 'research_contact', array['Post-production system ownership', 'Distributed creative handoffs', 'Senior producer language'], array['Could still be scoped as hands-on video execution', 'Hiring manager not obvious'], 'Lead with the difference between making videos and keeping post production from eating itself.', array['Surface distributed production systems', 'Use agency/tech launch examples', 'Name version-control and review-cycle fixes'], true),
  ('job-nova-dpm', 'default', 'company-nova', 'lever-103', 'lever', 'https://example.com/source/nova-dpm', 'https://example.com/apply/nova-dpm', 'Design Program Manager, Brand Systems', 'Nova Systems Studio', 'Remote, North America', 'remote', 'full-time', 'Design', 145000, 170000, '$145k-$170k', 'Run cross-functional brand systems work with design, content, product marketing, and creative operations partners.', '2026-06-03T09:10:00-06:00', '2026-06-04T08:22:00-06:00', 'archived', 78, 'B', 'Worth reviewing: good design-program shape, but title may skew toward classic DPM rituals.', array['Check scrum/process load', 'Salary floor depends on band'], 'review', array['Design systems', 'Brand operations', 'Cross-functional creative work'], array['May be too process-heavy', 'Could sit far from production authority'], 'Position as a creative ops person who can make design systems ship without making designers live inside status meetings.', array['Emphasize design/brand system launches', 'Reduce producer-only language', 'Keep operations voice sharp'], true),
  ('job-chain-ops', 'default', 'company-chain', 'html-chain-88', 'html', 'https://example.com/source/chain-ops', 'https://example.com/apply/chain-ops', 'Marketing Operations Lead', 'Chainlight', 'Remote', 'remote', 'contract', 'Marketing', 0, 0, '$120/hr contract', 'Coordinate campaign operations, content calendar, creator partnerships, paid channels, and social reporting for a web3 launch.', '2026-05-18T10:30:00-06:00', '2026-06-04T08:27:00-06:00', 'archived', 54, 'C', 'Possible but thin: remote and web3 fit, but rate is under target and the role leans marketing execution.', array['Below freelance floor', 'Performance/social skew', 'Older than 14 days'], 'review', array['Remote web3 launch', 'Campaign operations'], array['Too much performance marketing', 'Low authority signal'], 'Only pursue if there is hidden production ownership behind the generic marketing ops title.', array['Keep crypto/launch work visible', 'Do not over-invest without contact confirmation'], false),
  ('job-left-field-ep', 'default', 'company-left-field', 'ashby-990', 'ashby', 'https://example.com/source/lfl-ep', 'https://example.com/apply/lfl-ep', 'Executive Producer', 'Left Field Labs', 'Hybrid, Los Angeles', 'hybrid', 'full-time', 'Production', 170000, 190000, '$170k-$190k', 'Lead production delivery for interactive campaigns and technical creative teams.', '2026-06-04T08:01:00-06:00', '2026-06-04T08:24:00-06:00', 'archived', 0, 'skip', 'Do-not-apply company. Keep visible only so the system does not keep rediscovering it as tempting.', array['do-not-apply company', 'hybrid location'], 'skip', array['Title and salary would otherwise fit'], array['Explicit do-not-apply status', 'Hybrid requirement'], 'No outreach unless manually reactivated.', array['No action'], false)
on conflict (id) do nothing;
