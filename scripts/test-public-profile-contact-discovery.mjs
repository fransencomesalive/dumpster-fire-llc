...
// 2. A well-formed response with a functional lead + 3 contacts -> generated, no gap-fill call.
...
assert.equal(calls, 1, "no gap-fill call when >=3 contacts incl. a functional lead");
...
const gapped = await createOpenAIHumanPathProvider({
  callModel: async () => { gapCalls += 1; return gapCalls === 1 ? thinResponse : gapResponse; },
})({ pursuit, job });
assert.equal(gapCalls, 2, "gap-fill call fires when first pass is thin");
