...
const outDir = "/private/tmp/scans-batch1-candidate-preview-build";
const compileArgs = [
  "tsc",
  "--target",
  "ES2022",
  ...
  "--outDir",
  outDir,
  "scripts/preview-dumpster-fire-batch1-candidate-config.ts",
  "app/scans/store.ts",
...
run("npx", compileArgs);
run("node", [path.join(outDir, "scripts/preview-dumpster-fire-batch1-candidate-config.js"), ...process.argv.slice(2)], {
  env: {
    ...process.env,
