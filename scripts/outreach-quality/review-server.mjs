...
function saveFeedback(versionId, jobId, patch) {
  const path = resolve(dataDir, `feedback-${versionId}.json`);
  const current = readJson(path, { versionId, items: {} });
...
const server = createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/feedback") {
    ...
    const saved = saveFeedback(b.versionId, b.jobId, {
...
async function save(jobId, patch){
  ...
  const items=(STATE.feedback[CUR]=STATE.feedback[CUR]||{items:{}}).items;
